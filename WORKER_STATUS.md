...
You'll also need:
- `DATABASE_URL` set as an environment variable — ask Michele for the dev connection string
...
When storage is ready — if score >= 0.6 and needs_entity_extraction is true, hand the event off to Kaif's extractor and write a KnowledgeClaim to the knowledge_nodes table. The worker loop is already structured for this, it just needs that block added.
